...
void Defuzzy () {
  ...
  ruleMin[3] = min(suhu[0],kelembaban[2]);
  
  ruleMin[4] = min(suhu[1],kelembaban[0]);
  ruleMin[5] = min(suhu[1],kelembaban[1]);
  ruleMin[6] = min(suhu[1],kelembaban[2]);

  ruleMin[7] = min(suhu[2],kelembaban[0]);
  ruleMin[8] = min(suhu[2],kelembaban[1]);
  ruleMin[9] = min(suhu[2],kelembaban[2]);
...
//  a2 = (RuleMax*(90-45))+45;
  ...
  if(sensorSuhu <= 25.00 && sensorKelembaban >= 95.00  ){
    Serial.println("normal normal");
      a1 = ((RuleMin*(90-45))-90)*(-1);
      a2 = (RuleMax*(90-45))+45;
  }else if( sensorSuhu >= 29.00 && sensorKelembaban <= 65.00){
